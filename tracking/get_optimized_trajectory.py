...
def attach_origin_fields_to_trajectory(saved_path, env):
    """
    저장된 trajectory 파일에 reference spline의 *_origin 필드를 복사합니다.
    - convert_real_trajectory_to_spline.py에서 spline 파일에 추가한 원본 필드를 그대로 유지하기 위함.
    """
    try:
        if env is None or not hasattr(env, "_trajectory_manager") or env._trajectory_manager is None:
            return
        tm = env._trajectory_manager
        # spline을 사용하지 않으면 origin 정보가 없으므로 종료
        if not getattr(tm, "_use_splines", False):
            return
        ref_name = getattr(tm, "spline_name", None)
        ref_dir = getattr(tm, "_spline_dir", None)
        if ref_name is None or ref_dir is None:
            return
        ...
        for field in origin_fields:
            if field in ref_data:
                # 덮어써도 무방하지만, 이미 동일 필드가 있으면 그대로 둡니다.
                if saved_data.get(field) != ref_data[field]:
                    saved_data[field] = ref_data[field]
                    updated = True
        
        # selected_idx도 복사
        if 'selected_idx' in ref_data:
            if saved_data.get('selected_idx') != ref_data['selected_idx']:
                saved_data['selected_idx'] = ref_data['selected_idx']
                updated = True

        if updated:
            with open(saved_path, 'w') as f:
                f.write(json.dumps(saved_data, default=np_encoder, sort_keys=True))
                f.flush()
...
def rollout_single_worker_manually():
    ...
    saved_count = 0  # 저장된 trajectory 파일 개수 추적
    start = time.time()
    ...
    while True:
        # 목표 횟수 도달 시 종료 (최우선)
        if args.target_count is not None and saved_count >= args.target_count:
            logging.info(f"목표 저장 횟수({args.target_count})에 도달했습니다. 종료합니다.")
            break
        
        # episodes 제한이 있고, target_count가 없을 때만 episodes 제한 적용
        # target_count가 있으면 목표 횟수를 만족하기 위해 반복
        if args.episodes and args.target_count is None:
        ...
        stored_trajectory_file = None  # 저장된 trajectory 파일 경로 추적
        while not done:
            steps = steps + 1
            if args.store_network_data:
            ...
            if args.store_metrics:
                ...
                # 저장된 trajectory 파일 경로 추적
                if done and 'stored_trajectory_file' in info:
                    stored_trajectory_file = info['stored_trajectory_file']
        ...
        # store_trajectory가 활성화된 경우: 저장된 파일에 *_origin 필드를 보존/추가
        if args.store_trajectory and stored_trajectory_file and os.path.exists(stored_trajectory_file):
            attach_origin_fields_to_trajectory(stored_trajectory_file, env)
        ...
        # store_trajectory가 활성화되어 있고 target_duration이 설정된 경우
        # episode 종료 후 trajectory duration을 확인하여 조건에 맞지 않으면 저장된 파일 삭제
        if args.store_trajectory and args.target_duration is not None:
            if stored_trajectory_file and os.path.exists(stored_trajectory_file):
                # TERMINATION_SPLINE_DEVIATION인 경우 파일 삭제하고 카운트 증가 안 함
                if termination_reason == TERMINATION_SPLINE_DEVIATION:
                    os.remove(stored_trajectory_file)
                    logging.info(f"Episode {episodes_sampled}: TERMINATION_SPLINE_DEVIATION으로 종료됨. "
                               f"파일 삭제: {stored_trajectory_file} (카운트 증가 안 함)")
                # trajectory duration 확인
                elif trajectory_duration < args.target_duration:
                    # 조건 만족: 파일 유지
                    saved_count += 1
                    logging.info(f"Episode {episodes_sampled}: Duration {trajectory_duration:.3f}s < {args.target_duration:.3f}s (목표 시간). "
                               f"파일 유지: {stored_trajectory_file}. 저장된 파일 수: {saved_count}/{args.target_count if args.target_count else 'N/A'}")
                    print(f"Episode {episodes_sampled}: 파일 유지: {stored_trajectory_file}")
                else:
                    # 조건 불만족: 파일 삭제
                    os.remove(stored_trajectory_file)
                    logging.info(f"Episode {episodes_sampled}: Duration {trajectory_duration:.3f}s >= {args.target_duration:.3f}s (목표 시간). "
                               f"파일 삭제: {stored_trajectory_file}")
            elif args.store_trajectory:
                logging.warning(f"Episode {episodes_sampled}: 저장된 trajectory 파일 경로를 찾을 수 없습니다.")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--target_duration', type=float, default=None,
                        help='목표 trajectory duration (초). 이 값 미만일 때만 파일 유지')
    parser.add_argument('--target_count', type=int, default=None,
                        help='목표 저장 횟수. 이 횟수만큼 저장되면 종료')
    parser.add_argument('--output_dir', type=str, default=None,
                        help='저장 경로. 지정하면 해당 경로에 trajectory_data, env_config.json 등을 저장')
    ...
    # output_dir이 지정되면 해당 경로를 evaluation_dir로 사용
    if args.output_dir is not None:
        evaluation_dir = args.output_dir
        logging.info(f"저장 경로 지정: {evaluation_dir}")
    elif args.evaluation_dir is None:
    ...
    # output_dir이 지정되면 env_config에 evaluation_dir 설정하고 use_custom_evaluation_dir 플래그 설정
    if args.output_dir is not None:
        env_config['evaluation_dir'] = evaluation_dir
        env_config['use_custom_evaluation_dir'] = True
