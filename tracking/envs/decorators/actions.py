...
from tracking.utils import trajectory_plotter
from tracking.utils.braking_trajectory_generator import BrakingTrajectoryGenerator


class AccelerationPredictionBoundedJerkAccVelPos(ABC, TrackingBase):
    ...
    def __init__(self,
        ...
        if self._action_preprocessing_function is None:
            ...
            self.action_space = Box(low=-np.inf, high=np.inf, shape=(self._num_manip_joints,), dtype=np.float32)
        ...
        self._set_highest_action_to_one = set_highest_action_to_one
    ...
    def _add_baseline_position_to_plot(self, baseline_position_is, baseline_velocity_is, baseline_acceleration_is):
        if self._spline_use_toppra_baseline and (self._plot_trajectory or self._save_trajectory_plot):
            self._trajectory_plotter.add_baseline_value(baseline_position_is, baseline_velocity_is,
                                                        baseline_acceleration_is)
    ...
    def _compute_controller_setpoints_from_action(self, action):
        ...
        preprocessed_action = np.copy(action)
        ...
        if self._set_highest_action_to_one and \
                not (self._use_splines and self._trajectory_manager.spline_close_to_end):
            if abs(np.max(action)) >= abs(np.min(action)):
                action[action == np.max(action)] = 1.0
            else:
                action[action == np.min(action)] = -1.0
        ...
        return self._end_acceleration, controller_setpoints, obstacle_client_update_setpoints, \
            preprocessed_action, info, robot_stopped
